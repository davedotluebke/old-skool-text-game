...
class Map(thing.Thing):
                ...
                if self.location.location.id not in self.visited_rooms:
                    for i in self.prev_player_location.exits:
                        if self.prev_player_location.exits[i] == self.location.location.id:
                            proposed_coordinates = (self.coordinates[0] + directions[i][0], self.coordinates[1] + directions[i][1], self.coordinates[2] + directions[i][2]) # update the coordinates to the new room
                            if proposed_coordinates in self.map.keys():
                                # the coordinates are already used, insert a row / column
                                # call proposed_coordinate components (p_x, p_y, p_z):
                                # call coordinates of this room (r_x, r_y, r_z):
                                ...
                                self.coordinates = proposed_coordinates
    ...
    def print_map(self):
        ...
        for z in range(lowest_z, highest_z+1):
            level = "Level " + str(z) + ":\n"
            for y in range(highest_y, lowest_y-1, -1):
                line1 = ""
                line2 = ""
                line3 = ""
