...
class Chair(thing.Thing):
    ...
    def __init__(self, color):
        ...
        self.set_description('%s chair' % color, 'This confortable chair is covered in a %s felt-like surface.' % color)
        del Chair.avalible_colors[Chair.avalible_colors.index(color)]
        self.add_adjectives('confortable',color)
        self.sitting = None
    ...
    def stand(self, p, cons, oDO, oIDO):
        ...
        self.sitting = None
        self.emit("&nd%s stands up from the %s chiar" % (cons.user, self.color), ignore=[cons.user])
