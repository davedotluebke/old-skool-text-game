...
class WaterFountain(faucetThings.FaucetThing):
    def drink(self, p, cons, oDO, oIDO):
        if oIDO == self:
            ...
            self.emit('&nD%s graciously drinks from the water fountain.')
            return True
        else:
            return 'I don\'t see what you are trying tod drink!'
...
def clone():
    water_fountian = faucetThings.FaucetThing('fountain', __file__, 'simple water fountian', 'This is a simple copper water fountain, there is nothing abnormal about it.')
